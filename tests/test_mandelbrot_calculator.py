...
from mandelbrot_calculator import is_in_mandelbrot_set, MandelbrotSet
...
def test_mandelbrot_calculator() -> None:
    ...
    assert expected == observed, "make_grid failed"  # type: ignore[comparison-overlap]

    observed = ms.make_grid_parallel(-1, 1, -1, 1, 100)

    assert expected == observed, "make_grid_parallel failed"  # type: ignore[comparison-overlap]
